define(['exports'], (t) => {
    ...
    constructor(t, e, s) {
      super(
        ({ url: e }) => {
          ...
          if (s && (e.origin === location.origin || s.index === 0)) return s.slice(1);
        },
        e,
        s,
      );
  ...
  class o {
    ...
    addFetchListener() {
      self.addEventListener('fetch', (t) => {
        const { request: e } = t;
        const s = this.handleRequest({ request: e, event: t });
        s && t.respondWith(s);
      });
    }
    ...
    handleRequest({ request: t, event: e }) {
      ...
      const { params: i, route: r } = this.findMatchingRoute({
        event: e,
        request: t,
        sameOrigin: n,
        url: s,
      ...
      try {
        a = o.handle({
          url: s, request: t, event: e, params: i,
        });
      } catch (t) {
        a = Promise.reject(t);
      }
  ...
  const u = (t) => [h.prefix, t, h.suffix].filter((t) => t && t.length > 0).join('-');
  const l = (t) => t || u(h.precache);
  const f = (t) => t || u(h.runtime);
  function w(t, e) {
    const s = e();
  ...
  class y {
    constructor({ precacheController: t }) {
      (this.cacheKeyWillBeUsed = async ({ request: t, params: e }) => {
        const s = (e == null ? void 0 : e.cacheKey) || this.h.getCacheKeyForURL(t.url);
  ...
  class L {
    ...
    async fetch(t) {
      ...
      try {
        ...
        t = await fetch(
          n,
          n.mode === 'navigate' ? void 0 : this.l.fetchOptions,
        );
    ...
    async cacheMatch(t) {
      ...
      const r = await this.getCacheKey(e, 'read');
      const o = { ...i, cacheName: n };
    ...
    async cachePut(t, e) {
      ...
      const u = await self.caches.open(a);
      const l = this.hasCallback('cacheDidUpdate');
      const f = l
        ? await (async function (t, e, s, n) {
          const i = m(e.url, s);
          if (e.url === i) return t.match(e, n);
          const r = { ...n, ignoreSearch: !0 };
          const o = await t.keys(e, r);
          for (const e of o) if (i === m(e.url, s)) return t.match(e, n);
  ...
  class b {
    ...
    handleAll(t) {
      ...
      const e = t.event;
      const s = typeof t.request === 'string' ? new Request(t.request) : t.request;
      const n = 'params' in t ? t.params : void 0;
      const i = new L(this, { event: e, request: s, params: n });
      const r = this.q(i, s, e);
    ...
    async q(t, e, n) {
      ...
      try {
        if (((i = await this.L(e, t)), !i || i.type === 'error')) throw new s('no-response', { url: e.url });
      } catch (s) {
        if (s instanceof Error) for (const r of t.iterateCallbacks('handlerDidError')) if (((i = await r({ error: s, event: n, request: e })), i)) break;
        if (!i) throw s;
      }
  ...
  class C extends b {
    ...
    async O(t, e) {
      ...
      }
      {
        ...
        const o = !r || r === s;
        (n = await e.fetch(
          new Request(t, { integrity: t.mode !== 'no-cors' ? r || s : void 0 }),
        )),
        s
            && o
            && t.mode !== 'no-cors'
    ...
    N() {
      ...
      for (const [s, n] of this.plugins.entries()) {
        n !== C.copyRedirectedCacheableResponsesPlugin
          && (n === C.defaultPrecacheCacheabilityPlugin && (t = s),
          n.cacheWillUpdate && e++);
      }
      e === 0
        ? this.plugins.push(C.defaultPrecacheCacheabilityPlugin)
        : e > 1 && t !== null && this.plugins.splice(t, 1);
    }
  }
  ...
  (C.copyRedirectedCacheableResponsesPlugin = {
    cacheWillUpdate: async ({ response: t }) => (t.redirected ? await R(t) : t),
  });
  class E {
    constructor({
      cacheName: t,
    ...
    addToCacheList(t) {
      ...
      for (const n of t) {
        ...
        const r = typeof n !== 'string' && n.revision ? 'reload' : 'default';
